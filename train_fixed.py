...
class DataGenerator(Sequence):
    ...
    def array_to_normalized(self, img, means_bands, stds_bands):
        for b in range(0, img.shape[2]):
            img[b] = self.normalize(img[b], means_bands[b], stds_bands[b])

        return img

    
    def normalize(self, band, mean, std):
        """
    ...
    def __data_generation(self, list_patches_temp):
        ...
        for i in list_patches_temp:
            ...
            for s2 in i.extracted_s2:
                ...
                tmp = tiff.imread(tmp_path_s2).astype(int)
                tmp = self.array_to_normalized(tmp, self.means, self.stds)
                img.append(tmp)
...
stds_s2_total = [1026.9945542926912,
                 1026.8037708003687,
                 1026.756964647373,
                 1026.7293140071263,
                 1026.67839686092,
...
means_s2_total = [1669.8943928898348,
                  1669.9777603227067,
                  1669.9158340904905,
                  1669.8337722337887,
                  1669.7853136108606,
...
train_generator = DataGenerator(training, means_s2_total, stds_s2_total, path_s2, path_gr, n_classes, batch_size=batch_size, shuffle=True, dataAugment=False)
